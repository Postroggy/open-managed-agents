...
stateDiagram-v2
    [*] --> Active: 创建工作区
    Active --> Archived: POST /archive (id != default 且 != current)
...
归档端点对两种情况返回 `409 Conflict`，前后端各做一层防护：

| 场景 | 后端 | 前端 |
|------|------|------|
| `default` 工作区 | `id == "default"` → 409 `cannot_archive_default_workspace` | DropdownMenu 归档项 disabled |
| 当前会话绑定的工作区 | `principal.WorkspaceExternalID == id` → 409 `cannot_archive_current_workspace` | `workspace.id === activeWorkspaceId` 时归档项 disabled |
...
| 404 | 工作区不存在或不属于该组织（组织隔离） |
| 409 | 目标是 `default` 工作区，或当前会话绑定的工作区 |
