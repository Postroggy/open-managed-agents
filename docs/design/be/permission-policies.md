...
本仓库实现说明：`stop_reason.event_ids` 保存的是阻塞 `agent.tool_use` / `agent.mcp_tool_use` public event 的 `id`（`sevt_...`），不是 Claude Code worker 的原始 `request.tool_use_id`（`tool_...`）。为兼容早期客户端，`user.tool_confirmation.tool_use_id` 仍接受旧 worker `tool_use_id`，但新客户端应按官方契约使用 `event_ids` 中的 public event id。
