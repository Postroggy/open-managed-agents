...
2. 将该阻塞请求投影成 Managed Agents public event 契约：
   ...
   - `stop_reason` 可继续保留 `tool_use_id`、`tool_name`、`request_id`、`session_thread_id` 等兼容扩展字段；新客户端应优先使用 `event_ids`。
...
- `always_ask` 生成的 `session.status_idle.stop_reason.event_ids` 包含阻塞 `agent.tool_use` / `agent.mcp_tool_use` 的 public event id。
