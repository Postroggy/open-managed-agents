...
> **⚠️ OMA 项目级覆盖声明 — 优先于本 skill 正文的所有 testify 建议**
>
> 本项目（Open Managed Agents）使用 Go 标准 `testing`，**不引入 testify**。当本 skill 正文建议 testify（assert / require / mock / suite）时，按下表映射处理：
>
> | skill 建议 | OMA 实际做法 |
> |---|---|
> | `assert` / `require` 断言 | `t.Errorf` / `t.Fatalf`（标准库） |
> | testify `mock` | 手写 fake，或实现接口的最小 stub（本项目现有风格） |
> | testify `suite` | 标准 `t.Run` 命名子测试 + table-driven |
> | `testifylint` 等 lint | 不适用，本项目用 `.golangci.yml` 自有配置 |
>
> 本 skill 的其余最佳实践**全部适用并应遵循**：table-driven、named subtests、`t.Parallel`、只测可观察行为不测实现细节、race detection（`go test -race`）、integration build tags、**失败场景先行**、保持单测快速。
>
> 依据：项目 `CLAUDE.md` 的测试要求（标准 `testing`、失败场景先行）。
